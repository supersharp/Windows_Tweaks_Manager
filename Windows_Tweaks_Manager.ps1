...
function Install-TerminalMenu {
    ...
    New-ItemProperty -Path $RegPath_PS1 -Name "(default)" -Value "Run with Terminal (Admin)" -Force | Out-Null
    ...
    foreach ($path in @($RegPath_BAT, $RegPath_CMD)) {
        ...
        New-ItemProperty -Path $path -Name "(default)" -Value "Run with Terminal (Admin)" -Force | Out-Null
